...
from util import (slice_gauss, map_range, cv2torch, random_tone_map,
...
class ExpandNetLoss(nn.Module):
    def __init__(self, loss_lambda=5):
        ...
        self.similarity = torch.nn.CosineSimilarity(dim=1, eps=1e-5)
        self.l1_loss = nn.L1Loss()
        self.loss_lambda = loss_lambda
        self.eps = 1e-5

    def forward(self, x, y):
        cosine_term = (1 - self.similarity(x + self.eps, y + self.eps)).mean()
